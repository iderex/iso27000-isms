...
`templates/registers/risk-register`. There is no template for a measure, and
under `presentations`, `trainings` and `mappings` there is nothing for this
topic today. That is no invitation to invent something.
